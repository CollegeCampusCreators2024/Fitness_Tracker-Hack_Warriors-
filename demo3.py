...
# Twilio API details
TWILIO_ACCOUNT_SID = 'use your accoutn sid'
TWILIO_AUTH_TOKEN = 'use your own auth token'
TWILIO_PHONE_NUMBER = 'use your own number'
USER_PHONE_NUMBER = 'use your twilio number'

# Nutritionix API details
NUTRITIONIX_APP_ID = 'use your api id'
NUTRITIONIX_API_KEY = 'use your api key'
NUTRITIONIX_API_URL = "Use your Api url"

SHEETY_API_URL = "Paste your Sheety url"

def send_sms(exercise_name, reps, calories_burned):
...
class ExerciseCounter:
    def __init__(self, exercise_name, user_weight, user_height, user_age, gender, frame_width=640, frame_height=480):
        ...
        self.engine.setProperty('volume', 1)  # Set volume to max
    ...
    def update_count(self, results, counter_placeholder):
        ...
        if self.is_active and angle1 > 120 and angle > 120:
            ...
            if self.count == 1:  # Only call API once to avoid too many requests
            ...
            # Start the timer only once, after the first rep
            if not self.timer_started:
                Timer(5, send_sms, [self.exercise_name, self.count, self.calories_burned]).start()
                self.timer_started = True

            # Log the data to Google Sheets using Sheety API after each rep
            if self.count % 15 == 0:
...
def main():
    ...
    if st.sidebar.button("Start Exercise"):
        ...
        st.session_state.exercise_counter = ExerciseCounter(exercise_name, user_weight, user_height, user_age, gender)
